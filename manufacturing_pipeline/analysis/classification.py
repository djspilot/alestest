...
# ===========================================================================
# Helper: Hollow tube OD/wall-thickness constancy (for Step 0.2)
# ===========================================================================
...
def _check_hollow_tube_consistency(solid, axis, vertices, core_sec) -> Dict[str, Any]:
    ...
    Check A (OD variation):
      (max_OD - min_OD) / max_OD > OD_VAR_MAX -> ANDERS.

    Check B (wall-thickness variation):
      t_i = (OD_i - ID_i) / 2 per section where an inner ring is available.
      (max_t - min_t) / max_t > WALL_VAR_MAX -> ANDERS.

    Notes:
    - Radial holes can break the inner ring on some sections. Those sections are
      skipped for wall-thickness evaluation.
    - OD is measured from outer polygon bounds and is robust to radial holes.
    """
    import numpy as np
    ...
    # End-biased sampling catches stepped/turned tube ends with fewer slices.
...
def _step_0_2_hollow_closed(solid) -> Optional[Step0Result]:
    ...
    # --- Check if RONDE_BUIS with constant diameter ---
    if _is_nearly_circle(outer, 0.90, 0.94) and _is_nearly_circle(inner, 0.90, 0.94):
        # For round hollow tubes: check diameter consistency and axial ratio
    ...
    # --- Check if RECHTHOEKIGE_KOKER ---
    is_rect_strict = _is_nearly_rectangle(outer) and _is_nearly_rectangle(inner)
    # Afgeronde hoeken (standaard RHS/EN 10219): outer bbox_fill kan tot 0.85 zakken door
    # rondingradius, maar convexity blijft 1.0. Inner is doorgaans strakker.
    is_rect_rounded = (
    ...
    if used_wire_fallback and not is_rect_rounded:
        # Extra tolerantie bij wire-fallback (afgeronde hoeken in XCAF-reconstructie)
        is_rect_rounded = (
